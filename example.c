    ...
    while (*toexit == false) {
        ...
        if (selected == -1) {
            ...
            if (packet_is_dtls(payload, len)) {
                ...
                if ((len < 0) &&
                    SSL_get_error(dtls->ssl, len) == SSL_ERROR_SSL) {
                    fprintf(stderr,
                            "DTLS failure occurred on dtls session %p due to "
                            "reason '%s'\n",
                ...
                if (dtls->type == DTLS_CONTYPE_EXISTING) {
                    ...
                    // SSL_is_init_finished(), print key material.
                    {
                        ...
                        fprintfinger(stdout, "Fingerprint of peer's cert is ",
                                     peercert);
                        X509_free(peercert);
                    }
                    ...
                    if (km == NULL) {
                        fprintf(stderr,
                                "Unable to extract SRTP keying material from "
                                "dtls session %p\n",
                                dtls);
                        break;
                    }
...
int main(int argc, char **argv)
{
    ...
    do {
        ...
        do {
            ...
            do {
                ret = mainloop(fd, sslcfg, &timeout, &exitflag,
                               server ? NULL : &raddr);
            } while (0);
            close(fd);
        } while (0);
